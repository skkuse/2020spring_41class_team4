{/* <script> */}
var course_list = $('#course_value').val();
var bk_list = [];
var book_list = document.getElementsByClassName('book_list');
console.log(book_list);

for (var i = 0; i < book_list.length; i++){
	bk_list.push(book_list[i].value);
	console.log(book_list[i].value);
}
...
for(var i in course_list){
	...
	$.ajax({
		url : '/rest/courses/' + course_list[i] + '/',
		method:'GET',
		dataType:'json',
		async:false,
		...
		var second_time = data.second_time;
		var first_end, second_end;
		var week = data.week;
		var cnt = 2;
		var daydict = {"월" : "Monday", "화" : "Tuesday", "수" : "Wednesday", "목" : "Thursday", "금" : "Friday"};
		var weeka = daydict[week[0]];
		var weekb = daydict[week[2]];
		cnt = 1;
		a = first_time.split(":");
		...
		for(var i = 0; i < cnt; i++){
			...
			if(ul_list[j].className == weekday)
				ul = ul_list[j];
			};
			var datacontent="event-rowing-workout"; 
					var dataevent="event-" + ((i % 4)+1);
			var li = document.createElement('li');
			var a = document.createElement('a');
			a.setAttribute('data-start',start_time);
			a.setAttribute('data-end',end_time);
			a.setAttribute('course_id', course_id);
			a.setAttribute('c_number', c_number);
			a.setAttribute('data-content', cname);
			a.setAttribute('data-event', dataevent);
			a.setAttribute('professor', prof);
...
$(function() {
	function ScheduleTemplate( element ) {
		...
		this.modalProf = this.modal.getElementsByClassName('cd-schedule-modal__prof')[0];
	...
	ScheduleTemplate.prototype.initEvents = function() {
		var self = this;
		console.log("SingleEvent Len : " + this.singleEvents.length)
		for(var i = 0; i < this.singleEvents.length; i++) {
	...
	ScheduleTemplate.prototype.openModal = function(target) {
		...
		this.modalProf.textContent = target.getAttribute('professor');
		this.modal.setAttribute('data-event', target.getAttribute('data-event'));

		//update event content
		this.loadEventContent(target.getAttribute('course_id'));
		...
		if( mq == 'mobile' ) {
			...
			self.modalBodyBg.setAttribute('style', 'height:'+eventHeight+'px; width: 2 * '+eventWidth+'px; transform: scaleY('+HeaderBgScaleY+')');
